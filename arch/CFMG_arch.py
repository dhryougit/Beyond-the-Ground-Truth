...
def visualize_gaussian_masks(
    B=1, C=4, H=256, W=256, num_bins=5, device="cpu", sigma=0.1
):
    """
    Visualize and save the Gaussian masks generated by generate_gaussian_masks.
    ...
    Args:
        ...
        H (int): Image height.
        W (int): Image width.
        num_bins (int): Number of masks per channel.
        device (str): Torch device ('cuda' or 'cpu').
        sigma (float): Standard deviation for Gaussian.

    Saves:
        - Individual mask images: mask_0.png, mask_1.png, ..., mask_4.png
        - Grid image: masks_grid.png
    """
    # Generate masks
    masks = generate_gaussian_masks(
        B, C, H, W, num_bins, device, sigma
    )  # (B, C, num_bins, H, W)

    # Select masks for the first batch and first channel
    masks = masks[0, 0]  # (num_bins, H, W)

    # Normalize masks to [0, 1] for visualization
    masks = (masks - torch.amin(masks, dim=(1, 2), keepdim=True)) / (
        torch.amax(masks, dim=(1, 2), keepdim=True)
        - torch.amin(masks, dim=(1, 2), keepdim=True)
        + 1e-8
    )
    ...
    for i in range(num_bins):
        save_image(masks[i : i + 1], f"mask_{i}.png", normalize=False)

    # Save all masks as a grid
    save_image(
        masks,
        "/home2/dhryou/Git/OSEDiff_refinedgt/masks_grid.png",
        nrow=num_bins,
        normalize=False,
    )
...
class CFMG(nn.Module):
    ...
    def create_gaussian_weights(self, H, W):
        ...
        y = torch.arange(H, device=self.bin_centers.device).view(H, 1).repeat(1, W)
        x = torch.arange(W, device=self.bin_centers.device).view(1, W).repeat(H, 1)
        cy, cx = H / 2.0, W / 2.0
        dist = torch.sqrt((x - cx) ** 2 + (y - cy) ** 2)
        dist = dist / dist.max()  # normalize to [0,1]

        # Compute Gaussian weight for each bin center
        # Shape: (num_bins, H, W)
        diffs = dist.unsqueeze(0) - self.bin_centers.view(-1, 1, 1)
        gauss = torch.exp(-0.5 * (diffs / self.sigma) ** 2)
        # Normalize over bins
        weights = gauss / (gauss.sum(dim=0, keepdim=True) + 1e-8)
        return weights  # (num_bins, H, W)

    def create_ring_mask(self, H, W, num_rings):
        y, x = torch.meshgrid(
        ...
        dist = dist / max_dist  
        ring_indices = dist * num_rings  
        ring_indices_floor = ring_indices.floor().long()
        ring_indices_ceil = (ring_indices_floor + 1).clamp(max=num_rings - 1)
        weights = ring_indices - ring_indices_floor.float()  

        return ring_indices_floor, ring_indices_ceil, weights

    def get_ring_mask(self, H, W):
        """
        Get the ring mask, computing it only if it doesn't exist or the dimensions have changed.
        """
        if (
            self.ring_mask is None
            or self.mask_height.item() != H
            or self.mask_width.item() != W
        ):
            # Compute new ring mask
            self.ring_mask = self.create_ring_mask(H, W, self.num_bins)
            self.mask_height = torch.tensor(
                H, device=self.ring_mask.device, dtype=torch.long
            )
            self.mask_width = torch.tensor(
                W, device=self.ring_mask.device, dtype=torch.long
            )
    ...
    def forward(self, inp, c=0.5, inp_ref=None):
        ...
        # Generate Gaussian masks for all channels
        masks = generate_gaussian_masks(
        ...
        # Sum over num_bins to get one image per channel
...
class CFMG_element(nn.Module):
    ...
    def forward(self, inp, c=0.5, inp_ref=None):
        ...
        final_image = F.softmax(x, dim=1)  # (B, 4, H, W)
...
if __name__ == '__main__':
    ...
    net = NAFNet_cond(img_channel=7, output_channel=4, width=width, middle_blk_num=middle_blk_num,
